...
C04_COPYWRITER est la **frégate lourde** de la Porte 3. Elle forge le **text_payload** complet pour chaque angle :
...
Contrairement aux autres frégates qui dialoguent avec l'IRON (Claude sandbox) en 3 phases (prepare → IRON → finalize), C04 **contourne** ce pattern :
...
| `campaign_verdict.json` | C02_TYRANT_CAMP | JSON | ✅ |
| `source_specimen_<angle>.json` | C03_SOURCE_HUNTER (un par angle) | JSON | ✅ (N specimens) |
...
La clé API de C04_COPYWRITER est la **plus puissante** que le Warsmith possède. Elle est stockée dans :
...
- `CONTRACTS/anti_bullshit.md` (liens core)
- L'angle actif (depuis F02_ANGLESMITH)
- Le specimen source (depuis C03_SOURCE_HUNTER)
...
- **Amont** :
  - C02_TYRANT_CAMP (verdict, squelette référence)
  - C03_SOURCE_HUNTER (source specimen + segments)
  - F02_ANGLESMITH (angle actif)
  - ARCHIVUM complet (la matière à synthétiser)
- **Downstream** :
  - C05_PACKAGER (intègre `text_payload.json` dans `production_pack.json`)
