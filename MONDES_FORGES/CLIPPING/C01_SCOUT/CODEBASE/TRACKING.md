...
C01_SCOUT est la **frégate d'acquisition**. Elle prend en entrée les **assets fournis par la campagne** (URLs de vidéos longues, docs, briefs) et produit un `source_specimen.json` unifié consommé par C02_TYRANT_CAMP (Porte 1) et C03_SOURCE_HUNTER (Porte 3).
...
- `ARCHIVUM/campaign/reference_skeleton.json` — squelette viral extrait du clip de référence (pré-rempli pour C02)

---

## PATTERN D'EXÉCUTION

C01_SCOUT suit le pattern **3-phases standard** du core YOUTUBE :
...
Phase 3 : finalize
   ...
   → Met à jour liber_clipping.json (statut C01 = done)
```

---

## CONTRATS RÉFÉRENCÉS (ce que C01 doit lire)
...
- **Downstream** : C02_TYRANT_CAMP (consomme `source_specimen.json`), C03_SOURCE_HUNTER (consomme `assets[]`)

---

## HÉRÉSIES (ce que C01 ne fait jamais)
...
- ❌ Aucun scrape de concurrents — CAPTEURS s'en charge, pas C01
