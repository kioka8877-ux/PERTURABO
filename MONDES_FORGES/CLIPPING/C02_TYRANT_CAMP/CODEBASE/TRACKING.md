...
|---|---|---|---|
| `source_specimen.json` | C01_SCOUT | JSON | ✅ |
...
C02_TYRANT_CAMP et la frégate `TYRANT/` partagent la capacité "Oracle". C02 est spécialisée sur une campagne (mode réactif). TYRANT/ est le mode prospectif global (veille Démon dominants hors campagne).

| Mode | Qui | Quand | Sortie |
|---|---|---|---|
| Réactif | C02_TYRANT_CAMP | Pour chaque campagne fournie par le Warsmith | `campaign_verdict.json` |
...
C02 lit `ARCHIVUM/demons/` (synthèse des Démon cartographiés par TYRANT prospectif) pour identifier l'océan bleu de la campagne. **Démon de campagne** et **Démon veille** peuvent être identiques ou différents.
...
```
Phase 1 : prepare
   python tyrant_camp.py --prepare --specimen ../C01_SCOUT/OUT/source_specimen.json
...
Phase 3 : finalize
   ...
   → Met à jour liber_clipping.json (statut C02 = done, verdict disponible Porte 1)
```
...
C02 propose des **re-ciblages** sur des territoires adjacents non saturés, mais **uniquement sur la même source** que celle de la campagne (assets Whop).
...
- **Amont** : C01_SCOUT (`source_specimen.json`), CAPTEURS (`cartographie_écosystème` — optionnel mais recommandé), TYRANT prospectif (`ARCHIVUM/demons/`)
- **Downstream** :
  - F02_ANGLESMITH (consomme `verdict.json` pour la Porte 2 — forge les N angles sur `direct_analysis` + `blue_ocean_unlocked`)
  - C04_COPYWRITER (consomme `reference_skeleton` pour calibrer hooks)
