...
C06_TRACKER est la **frégate post-Pote 4**. Elle prend le relais après que les N `production_pack.json` ont été expédiés à OMNIS_WATCH. Pour chaque pack posté par le Warsmith, elle :
...
6. À la fermeture de campagne (déclarée par le Warsmith), agrège les résultats cumulés et active la pondération future de F02_ANGLESMITH (seuil 50 packs).
...
|---|---|---|---|
| `production_pack_<angle>.json` (N) | C05_PACKAGER (Porte 4) | JSON N | ✅ |
...
- **`learnings.json` cumul < 50 packs exécutés** : poids nul. F02_ANGLESMITH ne pondère pas les angles (tous égaux, neutres).
...
F02_ANGLESMITH lit `angle_performance[*].weight` pour pondérer le sélection d'angles.

---

## PATTERN D'EXÉCUTION

C06 ne fait pas appel à l'IRON ni au modèle premium. C'est une frégate purement déterministe qui:
...
- **Amont** : C05_PACKAGER (production packs avec submission_checklist imbriquée), Warsmith (saisies de vues/payouts)
- **Downstream** :
  - `ARCHIVUM/learnings/learnings.json` (nourri)
  - F02_ANGLESMITH (lit les poids pour pondération — se déclenche seulement à partir de 50 packs cumulés)
...
- ❌ Auto-poster ou auto-submit (l'opérateur poste ; C06 logge seulement)
