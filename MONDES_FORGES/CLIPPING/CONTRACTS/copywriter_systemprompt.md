...
`copywriter_systemprompt.md` est le **system prompt** utilisé par C04_COPYWRITER à chaque exécution de Phase B (premium_generation).
...
4. Ce script :
   ...
   - Construit un meta-prompt qui demande au modèle premium 
     "Génère le system prompt final pour C04_COPYWRITER 
     à partir de toute cette matière"
   - Premium génère
   - Sauvegarde dans copywriter_systemprompt.md
5. Ce fichier est figé
6. À chaque opération C04, on utilise ce system prompt
```
...
- Contraintes : format de sortie JSON strict (cf. `text_payload_*.json` schema dans `C04_COPYWRITER/CODEBASE/TRACKING.md`)
