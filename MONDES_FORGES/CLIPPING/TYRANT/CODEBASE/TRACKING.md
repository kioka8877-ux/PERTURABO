...
`TYRANT/` (à la racine de MONDES_FORGES/CLIPPING/) est la **frégate-Oracle en mode prospectif**. À l'inverse de C02_TYRANT_CAMP (qui analyse une campagne fournie = mode réactif), TYRANT prospectif scanne le **wild clipping** (hors campagne) pour identifier des **Démon dominants** et cartographier leurs **océans bleus unlocked**.

Le Warsmith déclenche TYRANT prospectif sur commande, hors campagne active. La sortie nourrit `ARCHIVUM/demons/` que C02_TYRANT_CAMP lira ensuite pour proposer des océans bleus sur les campagnes futures.
...
### Canonical copies
- `ARCHIVUM/demons/<demon_id>.json` — un fichier par Démon, qui sera lu par C02_TYRANT_CAMP plus tard
...
Si C02_TYRANT_CAMP tente d'empiler 2 couches, elle doit rejeter.
...
- **Downstream** : `ARCHIVUM/demons/` (nourri) → C02_TYRANT_CAMP (lit)
