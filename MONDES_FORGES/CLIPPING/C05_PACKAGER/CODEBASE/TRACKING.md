...
C05_PACKAGER est la **frégate finale de la Porte 4**. Elle assemble tous les artefacts produits par C01 → C04 et forge les N `production_pack.json` qui seront consommés par OMNIS_WATCH.
...
|---|---|---|---|
| `angles.json` | F02_ANGLESMITH | JSON | ✅ |
| `source_specimen_<angle>.json` (N) | C03_SOURCE_HUNTER | JSON N | ✅ |
| `text_payload_<angle>.json` (N) | C04_COPYWRITER | JSON N | ✅ |
| `verdict.json` | C02_TYRANT_CAMP | JSON | ✅ |
| `reference_skeleton.json` (style extrait) | C01_SCOUT/C04 → `reference_style` | JSON | ✅ |
...
Active, pas passive. À chaque pack, C05 génère une checklist que C06_TRACKER activera après Porte 4 :
...
"submission_checklist": {
  "items": [
    ...
    {"id": "log_link_c06", "label": "Logger le lien dans C06_TRACKER", "status": "pending"},
...
```
python packager.py --assemble --angles ../C02/OUT/angles.json
# Pour chaque angle :
...
C05 ne fait **pas** appel à l'IRON — c'est un enchaînement purement déterministe d'assemblage JSON. Pas de "préparation de prompt". La tâche est mécanique : fusion de JSONs.
...
- **Downstream** : 
  ...
  - C06_TRACKER (lit `submission_checklist` imbriquée, l'active après Porte 4)

---

## HÉRÉSIES

- ❌ Inclure un asset non-C03 dans `SOURCE`
- ❌ Inclure un texte non-C04 dans `TEXT_PAYLOAD`
- ❌ Omettre `reference_style` (matière première brute à transmettre à OMNIS_WATCH)
- ❌ `source_permission` != `"campaign_provided"` = hérésie
- ❌ Inclure des hashtag/caption non validés par C04 → C05 ne forge rien, il assemble
