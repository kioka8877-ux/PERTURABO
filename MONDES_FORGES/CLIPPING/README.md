...
|---|---|---|---|
| 1 | C02_TYRANT_CAMP (réactif + prospectif) | `verdict.json` (GO/NO-GO + `blue_ocean_unlocked`) | Warsmith |
| 2 | F02_ANGLESMITH (via C02 estratégia) | `angles.json` (N angles sur direct + océan bleu) | Warsmith |
| 3 | C03_SOURCE_HUNTER + C04_COPYWRITER | `source_specimen.json` + `text_payload.json` + `.md` | Warsmith + IRON ordonnancement |
| 4 | C05_PACKAGER | N `production_pack.json` → OMNIS_WATCH | Warsmith |

`C01_SCOUT` alimente les portes 1-2 (assets campagne). `C06_TRACKER` active la checklist après Porte 4 et nourrit `learnings.json`. `CAPTEURS` s'exécute **avant** Porte 1 sur commande Warsmith uniquement.
...
|---|---|---|---|
| C01 | SCOUT | Chasse les assets fournis par la campagne (jamais ailleurs — hérésie) | `source_specimen.json` |
| C02 | TYRANT_CAMP | Verdict campagne GO/NO-GO + identification océan bleu sur même source | `campaign_verdict.json` |
| C03 | SOURCE_HUNTER | Sélectionne les vidéos longues à clipper dans les assets | `source_specimen.json` |
| C04 | COPYWRITER | Forge le texte viral (3 titres + paragraphe + caption + hashtags + on-screen). **Frégate lourde** — clé API premium dédiée, contourne le pattern 3-phases | `text_payload.json` + `.md` |
| C05 | PACKAGER | Emballe le production pack final complet | `production_pack.json` |
| C06 | TRACKER | Checklist submission active (poster → Whop <1h → log → vues 1h+24h → flag → learnings) | `submission_log.json` |
...
F02_ANGLESMITH forge N angles répartis sur 2 zones :
...
```
Pendant la campagne :
   C06_TRACKER logge chaque pack posté + soumis + vues + payout
   
Fermeture campagne :
   Warsmith déclare "fin de campagne" → noté dans IW_CUSTOS.py (ledger)
   → C06 agrège les résultats → intègre learnings.json
   → campaign/ marqué "closed"
   → learnings.json enrichi (cumul)
   → si learnings > 50 packs cumulés → F02_ANGLESMITH commence
