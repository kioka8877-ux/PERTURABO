...
Fallback 1: yt-dlp --write-subs (anti-bot intégré).
Fallback 2: API timedtext de YouTube directement.
...
def _fetch_transcript_ytdlp(video_id: str, languages: list[str] = None) -> dict:
    """
    Fallback 1: utilise yt-dlp pour télécharger les sous-titres.
...
def _fetch_transcript_fallback(video_id: str, languages: list[str] = None) -> dict:
    """
    Fallback 2: récupère le transcript via l'API timedtext de YouTube directement.
...
def get_transcript(video_id: str, languages: list[str] = None) -> dict:
    ...
    Essaie youtube-transcript-api d'abord, fallback sur yt-dlp, puis API directe.
